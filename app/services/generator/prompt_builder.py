...
3. `dialogue_samples`
- 默认输出 1 个主对话样例。
- 对话优先保留教材原有轮次与说话人；证据充分时整理为 6-12 轮，证据较弱时允许更短，但不要为了凑轮次重复台词、重复场景或编造说话人。
- 每轮都必须包含 `speaker`, `text_en`, `text_zh`。
- 优先整理教材已有对话；只有在原文明显缺失个别承接句时，才允许基于当前单元词汇、句型和场景做最小幅度补足。
- `text_zh` 只写对应英文的直接中文释义，不要额外扩写。
- 不要重复相同或几乎相同的 turn；除非输入证据明确如此，否则不要连续输出同一 speaker 的多轮独白。
- `speaker` 必须来自输入证据中的真实称呼，无法确认时优先用稳定通用称呼如 `Boy`, `Girl`, `Teacher`，不要把整句误写成说话人。
- `source_excerpt` 必须截取自实际使用的对话证据，不要写泛化标题或凭空改写。
- 对话尽量覆盖至少 2 个重点句型和 4 个核心词汇，但准确性优先于覆盖率。
